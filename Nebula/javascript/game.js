...
Game = {
	...
	init: function() {
		...
		initMenu = function() {
			...
			Overlay = {
				draw: function(menuStage, selectedOption, offset) {	
					offset = offset || 0;
		...
		initLevel = function(levelNumber) {
			...
			gameLoop = function() {
				...
				if(Keyboard.escapeKey) {
					...
					return;*/
					location.reload(); // Cheap but effective.
				}
			...
			Overlay = {	
				...
				draw: function() {
					...
					drawHighscore = function(debug) {
						if(debug)
							ctx_overlay.fillText("HISCORE: " + levelHighscore, 10, 157); 	
						else {
							ctx_overlay.fillText("HISCORE: " + levelHighscore, 10, 37); 	
						}					
					}
					...
					drawBar = function(fontSize,barText, textX, textY, barSprite, barOverlaySprite, barEmptySprite, barX, barY, barWidthVariable) {
						...
						if(barWidthVariable <= 0) {
							barWidthVariable = 0.00000001;
						}
					...
					drawLevelStartMessage = function() {
						ctx_overlay.font = "" + 60 + "px fixedsys";
						ctx_overlay.textAlign = "center"; 
						ctx_overlay.textBaseline = 'middle';
						ctx_overlay.fillText(levelStartMessage, canvas.width/2 * 4, canvas.height/2 * 4 - 150);
						ctx_overlay.font = "" + 30 + "px fixedsys";
						for(var i = 0; i < levelTip.length; i++) { 
							if(i == 0) {
								ctx_overlay.fillText("TIP: " + levelTip[i], canvas.width/2 * 4, canvas.height/2 * 4 - 150 + 60); 
							} else {
								ctx_overlay.fillText(levelTip[i], canvas.width/2 * 4, canvas.height/2 * 4 - 150 + 60 + i*40); 	
							}
						}				
					}

					drawVictoryMessage = function() {
						ctx_overlay.font = "" + 160 + "px fixedsys";
						ctx_overlay.textAlign = "center"; 
						ctx_overlay.textBaseline = 'middle';
						ctx_overlay.fillText("VICTORY!", canvas.width/2 * 4, canvas.height/2 * 4 - 165); 	
						ctx_overlay.font = "" + 80 + "px fixedsys";	
						ctx_overlay.fillText("SCORE: " + GameInfo.score, canvas.width/2 * 4, canvas.height/2 * 4 - 165 + 114);	
						ctx_overlay.font = "" + 28 + "px fixedsys";						
						ctx_overlay.fillText("Press ESC to return to the main menu.", canvas.width/2 * 4, canvas.height/2 * 4 - 165 + 194);						
					}
					drawDefeatMessage = function() {
						ctx_overlay.font = "" + 120 + "px fixedsys";
						ctx_overlay.textAlign = "center"; 
						ctx_overlay.textBaseline = 'middle';
						ctx_overlay.fillText("GAME OVER", canvas.width/2 * 4, canvas.height/2 * 4 - 135);
						ctx_overlay.font = "" + 28 + "px fixedsys";	
						ctx_overlay.fillText("Press ESC to return to the main menu.", canvas.width/2 * 4, canvas.height/2 * 4 - 135 + 84);
					...
					if(DEBUG_MODE) {
						...
						drawGameTime();	
						drawDifficulty();
						drawHighscore(debug = true);	
					}	else {
						drawHighscore(debug = false);
					}
					if(levelHasStarted) {
						drawScore();
						drawComboCount();
					...
					if(levelHasStarted) {						
						...
						drawLevelStartMessage();					
					}

					if(levelHasEndedVictorious) {
						drawVictoryMessage();
					}
					if(levelHasEndedDefeated) {
						drawDefeatMessage();
					}
			...
			LevelHandler = {
				...
				endLevel: function(victorious) {
					...
					levelHasEnded = true;
					levelHasEndedVictorious = true;
					if(GameInfo.score > levelHighscore) {
						setCookie("hiscore", GameInfo.score, 36500);
					}
				},
				EventHandler: {
				...
				// Begins the process of spawning everything after the player has reached the y value for game start.
				levelStart: function() {
			...
			levelHighscore = parseInt(getCookie("hiscore"));
			if(!levelHighscore) {
				levelHighscore = 0;
			}
			levelHasEnded = false;
			levelHasEndedVictorious = false;
			levelHasEndedDefeated = false;

			levelStartMessage = "ENTERING " + level_data["level_" + levelNumber]["Metadata"]["LEVEL_NAME"].toUpperCase();
			tips = [
				"Press Q to fire your powerful secondary weapon.",
				"Beware of pink ghosts. They will charge at you\nwhen you are directly beneath them.",
				"You can avoid asteroids by hiding behind enemies.",
				"Keep an eye out for powerups!",
				"Keep moving when under heavy laser fire.",
				"Build up a huge combo by defeating\nmany enemies in succession."
			]

			levelTip = tips[Math.floor(random(0, tips.length))];
			if(levelTip.indexOf("\n") > 0) {
				levelTip = levelTip.split("\n");
			} else {
				levelTip = [levelTip];
			}

			//levelHighscore = var x = document.cookie;
		...
		Camera = {
			...
			shakeBriefly: function(intensity) {
