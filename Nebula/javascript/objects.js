...
function Object(data) {
	...
	this.checkCollisionWithObj = function(obj, useThisHitbox, useObjHitbox) {
		useThisHitbox = useThisHitbox || false;
		useObjHitbox = useObjHitbox || false;
...
function Player(data) {
	...
	this.bits_sprite = s_player_bits;
	this.bits_count = data.bit_count || 12;
	this.bits_speed = 1.4;
	this.bits_variance = 0.7;
	...
	this.movingToStart = data.movingToStart || false;

	this.death_sound = sound_alien_dead;
	...
	this.moveToVictory = function() {
		this.y -= 1;
		if(this.y < -16) {
			LevelHandler.endLevel(victorious = true);
		}
	}
	...
	this.checkMoves = function() {
		if(this.movingToVictory || this.movingToStart || dead) {
			return;
		}
...
function Enemy(data) {
	...
	this.shootPlayer = function(bulletType, shootX, shootY, silent) {
		shootX = shootX || 6;
		shootY = shootY || 12;
		silent = silent || false;
	...
	this.die = function(scoreless) {	
		scoreless = scoreless || false;	
		for(var i = 0; i < 12; i++) {
			new BulletParticleCircle(data = {x: this.x + this.sprite.w/2, y: this.y + this.sprite.h/2});					
		}
		for(var i = 0; i < this.bits_count; i++) {
...
function EnemyUFO(data) {
	...
	this.moveUFO = function(persistent) {
